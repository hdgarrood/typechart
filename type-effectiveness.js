...
function setNested(path, obj, value) {
    for (const el of path.slice(0, path.length - 1)) {
 ...
 * @returns {str is Type}
 */
const isType = (str) => types.some(t => t === str)

/**
 * @typedef {0 | 0.5 | 2} TypeEffectiveness
 * @typedef {typeof types[number]} Type
 * @type {Record<Type, Partial<Record<Type, TypeEffectiveness>>>}
 */
...
const defendingTypeEffectiveness = (function () {
    ...
    for (const atkType of types) {
        for (const defType of types) {
            if (attackingTypeEffectiveness[atkType][defType] != undefined) {
